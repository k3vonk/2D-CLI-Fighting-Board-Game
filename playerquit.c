...
void magic(struct player p[],int n,int attacked,int i);
int main(void)
{
	srand(time(NULL));
	int num, atk,attacked;
	int fstP = 0, secP = 1 , trdP = 2, frtP = 3, fftP = 4, sixP = 5;
	int magic_attack;
	num = play();
	...
	for(int i = 0; i < num; i++)
	{
		...
		magic_attack = players[i].smartness + 	players[i].magic_skills;
		
		printf("\n%s's MAGICAL ATTACK is %d",players[i].name, magic_attack);
		
		
		if(magic_attack>150){
			printf("\n%s wants to attack which player?",players[i].name);
			if(num == 2){
				printf("\n(0)First - (1)Second: ");
			}
			if(num == 3){
		...
		else if(players[i].action == 3){//quit
			printf("\n%s has left the game. \n", players[i].name);
				...
				/*	strcpy(players[i].name , players[i+1].name);
					players[i].type = players[i+1].type;
					players[i].life_points = players[i+1].life_points;
					...
					players[i].place->row = players[i+1].place->row;
					players[i].place->column = players[i+1].place->column;			*/	
			}
	...
	}while(players[num].life_points > 0 || num >1);	
...
void findSlots(int reqDist, int currDist,  struct slot * currSlot, struct slot * foundSlots, int * count,  bool explored[BOARD_SIZE][BOARD_SIZE]){
	...
	printf("FINDSLOTSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSS");
...
void magic(struct player p[],int n,int attacked,int i){
			...
			printf("\n%s Uses magic attack ",p[i].name);
			switch(attacked){
				case 0: if(fstP == i){
							printf("\nCANT ATTACK YOURSELF!");
							}
						else{
							p[fstP].life_points = p[fstP].life_points - ((0.5*p[i].magic_skills)+(0.2*p[i].smartness));
							printf("\nPlayer %d is attacking Player %d! ",i+1,fstP+1);
						}
				...
				case 1: if(secP == i){
							...
							p[secP].life_points = p[secP].life_points - ((0.5*p[i].magic_skills)+(0.2*p[i].smartness));
							printf("\nPlayer %d is attacking Player %d! ",i+1,secP+1);
						}	
					break;
				case 2:if(trdP == i){
							printf("\nCANT ATTACK YOURSELF!");
							...
							p[secP].life_points;
							printf("\nPlayer %d is attacking Player %d! ",i+1,trdP+1);
						}	
