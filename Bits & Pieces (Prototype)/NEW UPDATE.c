...
void magicAtk(struct player p[6], int num, int atker);
...
int main(void)
{
	...
	do{	
	printf("\n*****Round %d*****",rounds); //round iterations
	rounds++;	
	
	for(int i = 0; i < num; i++)
	{	
	if(players[i].life_points < 1){	//if life poitns calculations goes below 0	
					players[i].life_points = 0;		
		}
	
		
		int magic_atk = players[i].smartness + 	players[i].magic_skills;
		...
		switch(players[i].action){
			...
			if(magic_atk>150){                            //magic attack 
					printf("[MAGIC Attack] Choose a player\n");			
					magicAtk(players, num, i);
				...
				for(int i=0; i<BOARD_SIZE; i++){
					...
					* which points to slot at position (0, boarSize -1)*/
					...
					printf("\n\nFunction findSlotsinvoked(Adjacent):\n");
					printf("Player location [%d,%d] ",players[i].place->row,players[i].place->column);
					...
					int which;
						...
						for(i=0; i<count; i++){
							...
							printf("\n(%d, %d) ",foundSlots[i].row, foundSlots[i].column);
							for(x=0; x<num; x++){
								
								if((foundSlots[i].row == players[x].place->row) && (foundSlots[i].column == players[x].place->column)){
								printf("Player %d found at [%d,%d] ",x+1, players[x].place->row,players[x].place->column);
								near++;
							//	TF_atk = true;	
							}
					...
					printf("\nNearby players: %d",near);
					if (near==0){
						printf("\nAttack player: NO NEARBY");
					}
					if(near>0){
						for(i=0; i<count; i++){	
							for(x=0; x<num; x++){	
								if((foundSlots[i].row == players[x].place->row) && (foundSlots[i].column == players[x].place->column)){
								printf("\nPlayer %d can be attacked at [%d,%d] ",x+1, players[x].place->row,players[x].place->column);
								printf("\nATTACK WHO? ");                                                                                                //NEED WORK***************************************************************///////
									//scanf("%d", &which);
							//	printf("\nPlayer %d is attacking player %d at [%d,%d] ",i, which, players[which].place->row,players[which].place->column);
								}
					...
					printf("\nThere are %d slots surrounding", count);
			}				
		}
			break;
			case(3):
		...
		if(players[i].life_points == 0){ //eliminate
			printf("\n\n******************************************");
	...
	printf("\nThe Crossfire WINNER is %s",players[0].name);
}
...
int play()
{
	...
	printf("Enter the number of players(min 2,max 6): ");
	scanf("%d", &n);
...
void stats(struct player p[], int n){
	...
	printf("\n%-10s  Class \t%-40s\n", "Player Name","---------------Capabilities---------------\tLocation\tLife");
	...
	for(int i = 0; i < n; i++)
	{
		...
		switch(p[i].type){
			case(0):
				printf("%d.%-10s %-8s\tSm[%3d]  St[%3d]  MS[%3d]  L[%3d]  D[%3d]\t [%d,%d]\t\t Lf[%d]\n",i+1,p[i].name, "Elf",p[i].smartness, p[i].strength,p[i].magic_skills, p[i].luck, p[i].dexterity, p[i].place->row, p[i].place->column, p[i].life_points);
				break;
			case(1):
				printf("%d.%-10s %-8s\tSm[%3d]  St[%3d]  MS[%3d]  L[%3d]  D[%3d]\t [%d,%d]\t\t Lf[%d]\n",i+1,p[i].name, "Human",p[i].smartness, p[i].strength,p[i].magic_skills, p[i].luck, p[i].dexterity, p[i].place->row, p[i].place->column, p[i].life_points);
				break;
			case(2):
				printf("%d.%-10s %-8s\tSm[%3d]  St[%3d]  MS[%3d]  L[%3d]  D[%3d]\t [%d,%d]\t\t Lf[%d]\n",i+1,p[i].name, "Ogre",p[i].smartness, p[i].strength,p[i].magic_skills, p[i].luck, p[i].dexterity, p[i].place->row, p[i].place->column, p[i].life_points);
				break;
			case(3):
				printf("%d.%-10s %-8s\tSm[%3d]  St[%3d]  MS[%3d]  L[%3d]  D[%3d]\t [%d,%d]\t\t Lf[%d]\n",i+1,p[i].name, "Wizard",p[i].smartness, p[i].strength,p[i].magic_skills, p[i].luck, p[i].dexterity, p[i].place->row, p[i].place->column, p[i].life_points);
				break;
			default:
			break;
		}
...
void magicAtk(struct player p[6], int num, int atker){
	...
	for (i = 0; i < num; i++){
		if (i != atker){
			printf("Enter (%d) to inflict Magic damage to Player %d\n", i+1, i+1);
		}
	}
	scanf("%d", &victim);
	victim--;
	p[victim].life_points = p[victim].life_points - ((0.5*p[atker].magic_skills)+(0.2*p[atker].smartness));
}
